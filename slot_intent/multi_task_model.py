  ...
  def get_one(self, data, bucket_id, sample_id):
    ...
    for length_idx in xrange(decoder_size):
      ...
      for batch_idx in xrange(1):
       # We set weight to 0 if the corresponding target is a PAD symbol.7
        # The corresponding target is decoder_input shifted by 1 forward.
#        if length_idx < decoder_size - 1:
#          target = decoder_inputs[batch_idx][length_idx + 1]
#        print (length_idx)
  ...
  def get_one_test(self, data, bucket_id, sample_id):
    ...
    batch_weight = np.array([0], dtype=np.int32)
    batch_labels.append(batch_weight)
    # print('get_one_test labels:', batch_labels, '**********', batch_decoder_inputs)
    # batch_labels.append(
    #   np.array([labels[batch_idx][0]
    #             for batch_idx in xrange(1)], dtype=np.int32))
