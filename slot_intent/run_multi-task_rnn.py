...
def read_data(source_path, target_path, label_path, max_size=None):
  ...
  # print ('******************************************read_data _buckets',_buckets)
...
def read_test_data(source_path, target_path, label_path, max_size=None):
  data_set = [[] for _ in _buckets]
  with tf.gfile.GFile(source_path, mode="r") as source_file:
    with tf.gfile.GFile(target_path, mode="r") as target_file:
      with tf.gfile.GFile(label_path, mode="r") as label_file:
...
def train():
  ...
  data_dir = FLAGS.data_dir + '/training_data'
  in_seq_train, out_seq_train, label_train, in_seq_dev, out_seq_dev, label_dev, in_seq_test, out_seq_test, label_test, vocab_path, tag_vocab_path, label_vocab_path = data_utils.prepare_multi_task_data(
      data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size)
     
  result_dir = FLAGS.train_dir + '/training_test_results'
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        # Run evals on development/test set and print the accuracy.
            ...
            for bucket_id in xrange(len(_buckets)):
              ...
              # print('ref_label_list',ref_label_list)
...
def generate_result():
    ...
    data_dir = FLAGS.data_dir + '/test_data'
    in_seq_test, out_seq_test, label_test, vocab_path, tag_vocab_path, label_vocab_path = data_utils.prepare_multi_task_data_test(
        data_dir, FLAGS.in_vocab_size, FLAGS.out_vocab_size)
    ...
    current_taging_test_out_file = result_dir + '/answer_slot.txt'
    current_label_test_out_file = result_dir + '/answer_intent.txt'
    ...
    with tf.Session() as sess:
        ...
        # dev_set = read_data(in_seq_dev, out_seq_dev, label_dev)
        # print ('dev set:',dev_set)
        test_set = read_test_data(in_seq_test, out_seq_test, label_test)


        def save_test_result(data_set, mode):  # mode: Eval, Test
            # Run evals on development/test set and print the accuracy.
            hyp_tag_list = list()
            hyp_label_list = list()
            print ('len(_buckets)',len(_buckets))
            for bucket_id in xrange(len(_buckets)):
                eval_loss = 0.0
                count = 0
                print ('len(data_set[bucket_id])',len(data_set[bucket_id]))
                for i in xrange(len(data_set[bucket_id])):
                    count += 1
                    encoder_inputs, tags, tag_weights, sequence_length, labels = model_test.get_one_test(
                        data_set, bucket_id, i)
                    tagging_logits = []
                    classification_logits = []
                    if task['tagging'] == 1:
                        ...
                        _, step_loss, classification_logits = model_test.classification_step(sess,
                                                                                             encoder_inputs,
                                                                                             labels,
                                                                                             sequence_length,
                                                                                             bucket_id, True)
                ...
                if task['tagging'] == 1:
                    save_result(hyp_tag_list, current_taging_test_out_file )
                elif task['intent'] == 1:
                    save_result(hyp_label_list, current_label_test_out_file)
                # print('word_list', word_list)
            return

        save_test_result(test_set, 'Test')

def main(_):
    # print('begin to trainningt****************************************************')
    # train()
    print ('begin to get predict result***************************************************')
    generate_result()
